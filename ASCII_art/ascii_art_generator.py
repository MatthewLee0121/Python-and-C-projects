...
import cv2, os, time, io, base64  #used in the MP4 conversion
...
if __name__ == '__main__':
    ...
    def get_ascii_art(image_data, rows, columns):
        ...
        for y in range(0, height, rows):
            for x in range(0, width, columns):
                ...
                # Normalize brightness to fit the range of ASCII characters
                brightness_index = int((brightness / 255) * (len(ascii_characters_by_density) - 1))
    ...
    def mp4_to_jpeg():
        ...
        Variables: \n
         ...
         selected_output_path -- Imports the global empty string so we can assign a value \n
         cap -- explination \n
         frame_count -- explination \n
         ret -- explination \n
         frame -- explination \n
         ascii_art -- explination \n
         ascii_art_list -- explination \n
        -----------------------------------------------------------------------------\n
        Overview: \n
        ...
        while True:
            try:
                ...
                ascii_art = get_ascii_art(frame, rows.get(), columns.get())
    ...
    def jpeg_to_string(ascii_art_list):
        ...
        ascii_label = tk.Label(
            ...
            font=("Courier New", font_size_var.get()),  # Adjust font size and font family as needed
            bg="#f0f0f0",
            justify="left"  # Adjust justification as needed
        )
        ascii_label.pack(pady=5, padx=5)  # Place label in the first row, first column
